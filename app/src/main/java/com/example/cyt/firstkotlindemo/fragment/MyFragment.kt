            ...
            val result=bg{
                ...
                for(x in uls){
                    ...
                    for (i in lis){
                        ...
                        val time_and_class=i.select("div.lesson-infor > div > div:nth-child(l) > dl >dd.mar-b8>em").text()

                        val entity=ItemEntity(title = title,url = item_url,describe = describe,image = image,time_and_class = time_and_class,is_like = false)
                        mList.add(entity)
                    }
                }
            }
